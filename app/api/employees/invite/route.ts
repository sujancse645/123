...
export async function POST(request:NextRequest){
  try{
    ...
    const subject=`Activate your ${company.name} PayProof account`;const message=`Hello ${first},\n\nYour PayProof account is ready. Activate ${companyEmail} using the secure link below. The link expires at ${new Date(expiresAt).toLocaleString('en-IN')}.\n\n${actionLink}\n\nNo permanent password is included in this email.`;
