...
export function CreateEmployeeAccountModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const create = async (event: React.FormEvent) => {
    ...
    const gmailTab=window.open('about:blank','payproof-gmail-compose');
    if(gmailTab){gmailTab.document.title='Preparing PayProof invitation…';gmailTab.document.body.innerHTML='<p style="font:16px system-ui;padding:24px">Creating employee account and preparing Gmail…</p>'}
    try {
      if(!authenticated){
        ...
        const appUrl=process.env.NEXT_PUBLIC_APP_URL||window.location.origin;const activationUrl=`${appUrl}/reset-password?invite=1&demoInvite=${encodeURIComponent(invitation.token)}`;const draft={to:form.personalEmail.trim(),subject:'Activate your PayProof employee account',body:`Hello ${firstName},\n\nYour PayProof employee account and contract are ready.\n\nCreate your password and complete your employee profile using this link:\n\n${activationUrl}\n\nRegards,\nPayProof HR`};setMailDraft(draft);if(gmailTab)gmailTab.location.replace(gmailComposeUrl(draft));else window.open(gmailComposeUrl(draft),'_blank','noopener,noreferrer');setSent(true);return}
