# PayProof — Demo Scenarios & Integration Guide
...
All demo user accounts use the default password: **`PayProof@360`**

| Role | Name | Email | Primary Features Tested |
|---|---|---|---|
| **`admin`** | Sudeesh K | `admin@payproof.demo` | Role Permissions Matrix, Biometric Devices, Audit Trail, System Config |
...
| **`payroll_manager`** | Rajesh Kulkarni | `payroll.mgr@payproof.demo` | Payruns Execution, Salary Structures, Statutory Rules, Overtime Policy, Reports |
| **`payroll_user`** | Neha Gupta | `payroll.user@payproof.demo` | Payruns Drafting, Bank Exports, Payslip Generation |
| **`employee`** | Aravind Krishnan | `employee.aravind@payproof.demo` | Check-in/out, Leave Requests, Payslips View, My Loans, Profile Updates |
| **`employee`** | Ananya Roy | `employee.ananya@payproof.demo` | Alternate Employee Persona for cross-approval testing |
...
1. Log in as `employee.aravind@payproof.demo` / `PayProof@360`.
...
1. Log in as `employee.aravind@payproof.demo`.
...
1. Log in as `payroll.mgr@payproof.demo`.
...
5. Log in as `employee.aravind@payproof.demo` to inspect the generated payslip and download the PDF.
...
1. Log in as `admin@payproof.demo`.
